...
contract SocialNetwork {
    ...
    mapping(uint256 => PrivacySetting) public privacySettings;
    
    function addProfileHash(uint256 id, string memory hash) public {        
        Profile memory profile = Profile(id, hash);
        PrivacySetting memory privacySetting = PrivacySetting(id, true, true);
        profiles[id] = profile;
        privacySettings[id] = privacySetting;
    }
    ...
    function getPrivacySetting(uint256 id) public view returns(bool, bool) {
        PrivacySetting memory privacySetting = privacySettings[id];
        return(privacySetting.isProfileVisible, privacySetting.arePostsVisible);
    }

    function editPrivacySetting(uint256 id, bool isProfileVisible, bool arePostsVisible) public {
        PrivacySetting memory privacySetting = PrivacySetting(id, isProfileVisible, arePostsVisible);
        privacySettings[id] = privacySetting;
    }
}